...
MODEL_DIR = Path("./models/td3_yearly_single")
BEST_MODEL_DIR = MODEL_DIR / "best"
CHECKPOINT_DIR = MODEL_DIR / "checkpoints"
LOG_DIR = Path("./logs/td3_yearly_single")
TB_DIR = Path("./tb/td3_yearly_single")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train TD3 on the yearly Park IES task.")
...
def apply_runtime_args(args: argparse.Namespace) -> None:
    ...
    MODEL_DIR = Path(f"./models/td3_yearly_single_{run_suffix}")
    BEST_MODEL_DIR = MODEL_DIR / "best"
    CHECKPOINT_DIR = MODEL_DIR / "checkpoints"
    LOG_DIR = Path(f"./logs/td3_yearly_single_{run_suffix}")
    TB_DIR = Path(f"./tb/td3_yearly_single_{run_suffix}")
    RESULT_DIR = Path(f"./results/td3_yearly_training_{run_suffix}")
    TRAINING_EXPORT_XLSX = RESULT_DIR / "td3_training_export.xlsx"
    TRAINING_STEP_DETAIL_CSV = RESULT_DIR / "td3_training_step_detail.csv"
    TRAINING_EPISODE_SUMMARY_CSV = RESULT_DIR / "episode_summary.csv"
    TRAINING_RUNTIME_SUMMARY_JSON = RESULT_DIR / "training_runtime_summary.json"
...
EPISODE_DERIVED_COLUMNS = {"episode_idx", "global_step_start", "global_step_end", "episode_reward_scaled", "final_ees_soc", "final_gt_power"}
...
def describe_step_column(column_name: str) -> tuple[str, str]:
    if column_name == "global_step":
        return "Global TD3 environment step counted during training.", "callback"
...
def describe_episode_column(column_name: str) -> tuple[str, str]:
    ...
    if column_name in {"global_step_start", "global_step_end"}:
        return f"Global TD3 step marker `{column_name}` for the completed episode.", "callback"
    if column_name in {"month", "day_of_year", "season", "split"}:
...
def describe_config_column(column_name: str) -> tuple[str, str]:
    if column_name.isupper():
        return f"Training constant `{column_name}` recorded for experiment reproducibility.", "train_TD3.py"
    if column_name in {"run_id", "timestamp_start", "training_duration_seconds", "training_duration_hms", "policy", "net_arch_pi", "net_arch_qf", "activation_fn", "device"}:
        return f"Run setup field `{column_name}` recorded for experiment reproducibility.", "train_TD3.py"
    if column_name.endswith("_path"):
        return f"Input data path `{column_name}` used for this run.", "train_TD3.py"
    if column_name.endswith("_cases"):
        return f"Dataset case count `{column_name}` observed at startup.", "YearlyCSVDataLoader"
    return f"Configuration value `{column_name}` captured for experiment reproducibility.", "IESConfig/train_TD3.py"
...
def import_runtime_modules():
    ...
    return {
        "pd": pd, "np": np, "th": th, "TD3": TD3, "BaseCallback": BaseCallback,
...
def main() -> None:
    ...
    checkpoint_callback = CheckpointCallback(save_freq=SAVE_FREQ, save_path=str(CHECKPOINT_DIR), name_prefix="td3_yearly_single", save_replay_buffer=False, save_vecnormalize=False)
    ...
    print(f"[startup] TD3 model device = {model.device}")
    ...
    print(f"训练 episode_summary CSV 已导出到: {TRAINING_EPISODE_SUMMARY_CSV.resolve()}")
