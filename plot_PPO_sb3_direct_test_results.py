...
DEFAULT_SUMMARY_CSV_PATH = BASE_DIR / "results" / "ppo_sb3_direct_test" / "daily_summary.csv"
DEFAULT_TIMESERIES_CSV_PATH = BASE_DIR / "results" / "ppo_sb3_direct_test" / "timeseries_detail.csv"
DEFAULT_OUTPUT_DIR = BASE_DIR / "results" / "ppo_sb3_direct_test" / "plots"
DEFAULT_TYPICAL_SUMMARY_PATH = BASE_DIR / "results" / "ppo_sb3_direct_test" / "typical_days_summary.csv"
DEFAULT_CASE_ANALYSIS_PATH = BASE_DIR / "results" / "ppo_sb3_direct_test" / "abnormal_case_summary.csv"
DEFAULT_ANALYSIS_REPORT_PATH = BASE_DIR / "results" / "ppo_sb3_direct_test" / "test_analysis_report.md"
...
def plot_summary_overview(summary_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig.suptitle("PPO test summary overview", fontsize=15)
...
def plot_test_diagnostics(summary_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig.suptitle("PPO test diagnostics", fontsize=15)
    fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.97])
...
def build_dispatch_title(case_index: int, day_of_year: int, month: int, season: str | None) -> tuple[str, str]:
    ...
    if season_text:
        title = f"PPO {season_text.capitalize()} case {case_index:02d} | day {day_of_year} | month {month:02d}"
    else:
        title = f"PPO case {case_index:02d} | day {day_of_year} | month {month:02d}"
    return prefix, title
...
def describe_abnormal_method(result: dict[str, object], metric_label: str) -> str:
    ...
    if method == "all_zero":
        return f"{metric_label} 全部接近 0，未识别到明显异常。"
...
def plot_daily_cost_penalty_anomalies(
    ...
    configs = [
        (axes[0], "total_system_cost", "PPO 测试日总成本异常定位", "总成本", COLOR_MAP["Cost"], cost_result),
        (axes[1], "total_penalties", "PPO 测试日总惩罚异常定位", "总惩罚", COLOR_MAP["Penalty"], penalty_result),
    ]
    ...
    for ax, metric_col, title, ylabel, color, result in configs:
        ...
        if method != "all_zero":
            ...
            ax.axhline(threshold, color="#444444", linestyle="--", linewidth=1.4, label=f"{threshold_label}: {threshold:.2f}")
...
def plot_daily_cost_breakdown(
    ...
    if specs:
        ...
        draw_empty_panel(ax, "PPO 测试日成本分解", "Missing cost component columns")
...
def plot_daily_penalty_breakdown(
    ...
    if specs:
        ...
        draw_empty_panel(ax, "PPO 测试日惩罚项分解", "No non-zero penalty columns found")
    ...
    *,
    ...
    lines.extend(
        [
            "",
            "### 高成本 case",
            *build_case_report_lines(pd.DataFrame(cost_result["flagged_df"]), "total_system_cost", COST_COMPONENT_SPECS, "total_system_cost"),
            "",
            "## 惩罚整体",
            "",
            ...
            "",
            "### 高惩罚 case",
            *build_case_report_lines(pd.DataFrame(penalty_result["flagged_df"]), "total_penalties", PENALTY_COMPONENT_SPECS, "total_penalties"),
        ]
    )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot compact PPO test figures aligned with test_PPO_sb3_direct.py outputs."
    )
...
def main() -> None:
    ...
    print(f"PPO test plots saved to: {args.output_dir}")
    if not args.skip_summary:
