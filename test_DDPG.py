...
DEFAULT_MODEL_PATH = Path("./models/ddpg_yearly_single/best/best_model.zip")

DEFAULT_OUTPUT_DIR = Path("./results/ddpg_yearly_test")
SUMMARY_FILENAME = "daily_summary.csv"
TIMESERIES_FILENAME = "timeseries_detail.csv"
RUNTIME_SUMMARY_FILENAME = "runtime_summary.json"
EXCEL_FILENAME = "ddpg_test_export.xlsx"

MODEL_PATH = DEFAULT_MODEL_PATH
RESULT_DIR = DEFAULT_OUTPUT_DIR
SUMMARY_CSV = RESULT_DIR / SUMMARY_FILENAME
TIMESERIES_CSV = RESULT_DIR / TIMESERIES_FILENAME
TEST_EXPORT_XLSX = RESULT_DIR / EXCEL_FILENAME
RUNTIME_SUMMARY_JSON = RESULT_DIR / RUNTIME_SUMMARY_FILENAME
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test DDPG on the yearly Park IES task.")
...
def apply_runtime_args(args: argparse.Namespace) -> None:
    ...
    if args.run_name is None and SEED == DEFAULT_SEED:
        ...
        MODEL_PATH = Path(f"./models/ddpg_yearly_single_{run_suffix}/best/best_model.zip")
        RESULT_DIR = Path(f"./results/ddpg_yearly_test_{run_suffix}")
...
DAILY_SUMMARY_COLUMNS = (
    "case_index", "month", "day_of_year", "season", "split",
    "total_reward", "total_reward_raw", "total_system_cost", "total_penalties", "total_cost_plus_penalty", "total_guide_reward",
...
def describe_step_column(column_name: str) -> tuple[str, str]:
    if column_name in STEP_METADATA_COLUMNS:
        return f"Test sample metadata field `{column_name}` for the current step.", "test_DDPG.py"
    ...
    if column_name.startswith("a_"):
        return f"Executed DDPG action component `{column_name}` after clipping.", "ParkIESEnv.step()"
...
def describe_summary_column(column_name: str) -> tuple[str, str]:
    if column_name in {"case_index", "month", "day_of_year", "season", "split"}:
        return f"Test sample metadata field `{column_name}` for the whole day.", "test_DDPG.py"
    if column_name.startswith("total_"):
        return f"Day-level aggregated metric `{column_name}` summed or integrated across the tested day.", "test_DDPG.py"
    if column_name.startswith("avg_"):
        return f"Day-level average power metric `{column_name}` over the tested day.", "test_DDPG.py"
    if column_name.startswith("final_") or column_name == "ees_soc_episode_init":
        return f"Day-level initial or terminal metric `{column_name}`.", "test_DDPG.py"
    return f"Daily summary metric `{column_name}`.", "test_DDPG.py"


def describe_config_column(column_name: str) -> tuple[str, str]:
    if column_name.isupper():
        return f"Test constant `{column_name}` recorded for reproducibility.", "test_DDPG.py"
    if column_name.endswith("_path"):
        return f"Filesystem path `{column_name}` used by this test export.", "test_DDPG.py"
    if column_name in {"run_id", "timestamp_start", "algorithm", "policy_class", "device"}:
        return f"Runtime metadata field `{column_name}` for this test run.", "test_DDPG.py"
    if column_name.endswith("_cases"):
        return f"Dataset split count `{column_name}` observed at startup.", "YearlyCSVDataLoader"
    return f"Environment or reward configuration field `{column_name}` recorded for reproducibility.", "IESConfig/test_DDPG.py"
...
def main() -> None:
    ...
    model = DDPG.load(str(MODEL_PATH), device=resolved_device)
    ...
    if summary_rows:
        ...
        grand_total_grid_buy_kwh = float(sum(item["total_grid_buy_kwh"] for item in summary_rows))
        grand_total_grid_sell_kwh = float(sum(item["total_grid_sell_kwh"] for item in summary_rows))
        grand_total_depart_energy_shortage_kwh = float(
            sum(item["total_depart_energy_shortage_kwh"] for item in summary_rows)
        )
        n_days = len(summary_rows)
        mean_total_system_cost = grand_total_cost / max(n_days, 1)
        mean_total_penalties = grand_total_penalties / max(n_days, 1)
        mean_total_cost_plus_penalty = (
            grand_total_cost + grand_total_penalties
        ) / max(n_days, 1)
        terminal_ees_feasible_days = int(
            sum(
                bool(item.get("ees_terminal_soc_feasible", False))
                and float(item.get("terminal_ees_shortage_kwh", 0.0)) <= ALERT_TOL
                for item in summary_rows
            )
        )
        terminal_ees_feasible_ratio = terminal_ees_feasible_days / max(n_days, 1)
        min_final_ees_soc = float(min(item["final_ees_soc"] for item in summary_rows))

        print("\n========== DDPG Test Set Summary ==========")
        ...
        print(f"mean_total_system_cost: {mean_total_system_cost:.6f}")
        print(f"mean_total_penalties: {mean_total_penalties:.6f}")
        print(f"mean_total_cost_plus_penalty: {mean_total_cost_plus_penalty:.6f}")
        print(f"terminal_ees_feasible_days: {terminal_ees_feasible_days}")
        print(f"terminal_ees_feasible_ratio: {terminal_ees_feasible_ratio:.6f}")
        print(f"min_final_ees_soc: {min_final_ees_soc:.6f}")
        print(f"sum_terminal_ees_shortage_kwh: {grand_total_terminal_ees_shortage_kwh:.6f}")
        print(f"sum_penalty_terminal_ees_soc: {grand_total_penalty_terminal_ees_soc:.6f}")
        print(f"total_depart_energy_shortage_kwh: {grand_total_depart_energy_shortage_kwh:.6f}")
        print(f"total_unmet_e: {grand_total_unmet_e:.6f}")
        print(f"total_unmet_h: {grand_total_unmet_h:.6f}")
        print(f"total_unmet_c: {grand_total_unmet_c:.6f}")
        print(f"total_grid_buy_kwh: {grand_total_grid_buy_kwh:.6f}")
        print(f"total_grid_sell_kwh: {grand_total_grid_sell_kwh:.6f}")
        if terminal_ees_feasible_days != n_days or grand_total_terminal_ees_shortage_kwh > ALERT_TOL:
            print("WARNING: DDPG does not fully satisfy EES terminal SOC constraint.")
    else:
