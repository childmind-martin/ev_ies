...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test PPO on the yearly Park IES task.")
...
def apply_runtime_args(args: argparse.Namespace) -> None:
    global SEED, RUN_NAME, REQUESTED_DEVICE, MODEL_PATH, RESULT_DIR, SUMMARY_CSV, TIMESERIES_CSV, CONFIG_CSV, TEST_EXPORT_XLSX, RUNTIME_SUMMARY_JSON
    ...
    if args.run_name is None and SEED == DEFAULT_SEED:
        ...
        MODEL_PATH = Path(f"./models/ppo_sb3_direct_{run_suffix}/best/best_model.zip")
        RESULT_DIR = Path(f"./results/ppo_sb3_direct_test_{run_suffix}")
