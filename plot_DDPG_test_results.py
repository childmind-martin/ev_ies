...
DEFAULT_SUMMARY_CSV_PATH = BASE_DIR / "results" / "ddpg_yearly_test" / "daily_summary.csv"
DEFAULT_TIMESERIES_CSV_PATH = BASE_DIR / "results" / "ddpg_yearly_test" / "timeseries_detail.csv"
DEFAULT_OUTPUT_DIR = BASE_DIR / "results" / "ddpg_yearly_test" / "figures"
DEFAULT_TYPICAL_SUMMARY_PATH = BASE_DIR / "results" / "ddpg_yearly_test" / "typical_days_summary.csv"
DEFAULT_CASE_ANALYSIS_PATH = BASE_DIR / "results" / "ddpg_yearly_test" / "abnormal_case_summary.csv"
DEFAULT_ANALYSIS_REPORT_PATH = BASE_DIR / "results" / "ddpg_yearly_test" / "test_analysis_report.md"
...
def plot_daily_cost_penalty_anomalies(
    ...
    fig.suptitle("DDPG 测试日异常总览", fontsize=16)
    axes[1].text(
        0.99,
        0.96,
...
def plot_summary_overview(summary_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig.suptitle("DDPG test summary overview", fontsize=15)
    fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.97])
...
def plot_test_diagnostics(summary_df: pd.DataFrame, output_dir: Path) -> None:
    ...
    fig.suptitle("DDPG test diagnostics", fontsize=15)
    fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.97])
...
def build_dispatch_title(case_index: int, day_of_year: int, month: int, season: str | None) -> tuple[str, str]:
    ...
    if season_text:
        title = f"DDPG {season_text.capitalize()} case {case_index:02d} | day {day_of_year} | month {month:02d}"
    else:
        title = f"DDPG case {case_index:02d} | day {day_of_year} | month {month:02d}"
    return prefix, title
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot compact DDPG test figures aligned with test_DDPG.py outputs."
    )
...
def main() -> None:
    ...
    print(f"DDPG test plots saved to: {args.output_dir}")
    if not args.skip_summary:
