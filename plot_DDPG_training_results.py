...
DEFAULT_TB_LOG_DIR = BASE_DIR / "tb" / "ddpg_yearly_single"
DEFAULT_EVAL_NPZ_PATH = BASE_DIR / "logs" / "ddpg_yearly_single" / "evaluations.npz"
DEFAULT_EPISODE_SUMMARY_CSV_PATH = BASE_DIR / "results" / "ddpg_yearly_training" / "episode_summary.csv"
DEFAULT_OUTPUT_DIR = BASE_DIR / "results" / "ddpg_yearly_training" / "figures"
...
EES_TERMINAL_KEYS = [
    ("final_ees_soc", "Final EES SOC", "#1F77B4"),
    ("terminal_ees_required_soc", "Required EES SOC", "#2CA02C"),
    ("episode_terminal_ees_shortage_kwh", "EES terminal shortage", "#D62728"),
    ("episode_penalty_terminal_ees_soc", "EES terminal penalty", "#9467BD"),
    ("ees_terminal_soc_feasible", "EES terminal feasible", "#7F7F7F"),
]
...
def load_episode_summary_series(episode_summary_csv_path: Path) -> Tuple[Dict[str, List[Tuple[int, float]]], Path | None]:
    if not episode_summary_csv_path.exists():
        return {}, None
    if pd is None:
        print("pandas is not available; episode-level training curves from CSV will be skipped.")
        return {}, episode_summary_csv_path

    try:
        episode_df = pd.read_csv(episode_summary_csv_path)
    except Exception as exc:
        print(f"Failed to read episode_summary from {episode_summary_csv_path}: {exc}")
        return {}, episode_summary_csv_path

    if "global_step_end" not in episode_df.columns:
        print(f"Episode summary CSV {episode_summary_csv_path} is missing 'global_step_end'.")
        return {}, episode_summary_csv_path
    ...
    return series, episode_summary_csv_path
...
def plot_training_summary(
    ...
    fig.suptitle("DDPG training summary", fontsize=15)
...
def plot_training_diagnostics(
    ...
    plot_multi_series_panel(
        ...
        scalar_series,
        EES_TERMINAL_KEYS,
        title="EES terminal SOC diagnostics",
        ylabel="SOC / kWh / penalty / feasible",
        smooth_window=EPISODE_SMOOTH_WINDOW,
    )

    fig.suptitle("DDPG training diagnostics", fontsize=15)
    fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.97])
    fig.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Plot compact DDPG training figures aligned with train_DDPG.py outputs."
    )
    ...
    parser.add_argument("--episode-summary-csv", type=Path, default=DEFAULT_EPISODE_SUMMARY_CSV_PATH)
...
def main() -> None:
    ...
    episode_scalar_series, episode_summary_path = load_episode_summary_series(args.episode_summary_csv)
